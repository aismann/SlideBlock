    ...
    if(i != data.size() - 1) {
      ...
      if(data.at(i)->typeObject == WIN_OBJECT) {
        if(data.at(i)->x1 == data.at(i)->x2) {
          sprintf(fileName, "target_vertical_%d_%d.png", modeGame, object->getHeightObject());
        }
        if(data.at(i)->y1 == data.at(i)->y2) {
          sprintf(fileName, "target_horizontal_%d_%d.png", modeGame, object->getLengthObject());
        }
      } else {
        if(data.at(i)->x1 == data.at(i)->x2) {
        ...
        if(data.at(i)->y1 == data.at(i)->y2) {
          sprintf(fileName, "horizontal_%d_%d.png", modeGame, object->getLengthObject());
        }
      }
