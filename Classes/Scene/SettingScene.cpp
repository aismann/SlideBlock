...
bool SettingScene::init() {
  ...
  lister->onTouchBegan = CC_CALLBACK_2(SettingScene::handleTouchBegan, this);
...
bool SettingScene::handleTouchBegan(Touch* mTouch, Event* pEvent){
  Vec2 posTouchBegin = mTouch->getLocation();
  
  if(posTouchBegin.y > positionMusicON.y - thumbSwitchMusic->getContentSize().height/2 &&
     posTouchBegin.y <  positionMusicON.y + thumbSwitchMusic->getContentSize().height/2) {
    if(posTouchBegin.x > (positionMusicON.x + positionMusicOFF.x)/2 &&
       posTouchBegin.x < positionMusicON.x && !isOnMusic) {
    ...
    if(posTouchBegin.x < (positionMusicON.x + positionMusicOFF.x)/2 &&
       posTouchBegin.x > positionMusicOFF.x &&isOnMusic) {
  ...
  if(posTouchBegin.y > positionThemeON.y - thumbSwitchTheme->getContentSize().height/2 &&
     posTouchBegin.y <  positionThemeON.y + thumbSwitchTheme->getContentSize().height/2) {
    if(posTouchBegin.x > (positionThemeON.x + positionThemeOFF.x)/2 &&
       posTouchBegin.x < positionThemeON.x && !isOnTheme) {
    ...
    if(posTouchBegin.x < (positionThemeON.x + positionThemeOFF.x)/2 &&
       posTouchBegin.x > positionThemeOFF.x && isOnTheme) {
